...
object JmsConsumer {
  ...
  def create(settings: JmsConsumerSettings): akka.stream.javadsl.Source[Message, JmsConsumerControl] =
    akka.stream.alpakka.jakartajms.scaladsl.JmsConsumer.apply(settings).mapMaterializedValue(toConsumerControl).asJava
  ...
  def textSource(settings: JmsConsumerSettings): akka.stream.javadsl.Source[String, JmsConsumerControl] =
    akka.stream.alpakka.jakartajms.scaladsl.JmsConsumer
      .textSource(settings)
  ...
  def objectSource(
    ...
    akka.stream.alpakka.jakartajms.scaladsl.JmsConsumer
      .objectSource(settings)
      .mapMaterializedValue(toConsumerControl)
      .asJava

  /**
   * Creates a source emitting [[akka.stream.alpakka.jakartajms.AckEnvelope AckEnvelope]] instances, and materializes a
   * control instance to shut down the consumer.
  ...
  /**
   * Creates a source emitting [[akka.stream.alpakka.jakartajms.TxEnvelope TxEnvelope]] instances, and materializes a
   * control instance to shut down the consumer.
  ...
  def browse(settings: JmsBrowseSettings): akka.stream.javadsl.Source[Message, NotUsed] =
    akka.stream.alpakka.jakartajms.scaladsl.JmsConsumer.browse(settings).asJava
