...
/**
 * Settings for [[akka.stream.alpakka.jakartajms.scaladsl.JmsConsumer.browse]] and [[akka.stream.alpakka.jakartajms.javadsl.JmsConsumer.browse]].
 */
...
object JmsBrowseSettings {
  ...
  val configPath = "alpakka.jakarta-jms.browse"
  ...
  /**
   * Reads from the default config provided by the actor system at `alpakka.jakarta-jms.browse`.
   *
  ...
  /**
   * Java API: Reads from the default config provided by the actor system at `alpakka.jakarta-jms.browse`.
   *
   * @param actorSystem The actor system
   * @param connectionFactory Factory to use for creating JMS connections.
   */
