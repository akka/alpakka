...
import akka.stream.alpakka.jakartajms.impl.InternalConnectionState.JmsConnectorStopping
import akka.stream.alpakka.jakartajms.{Destination, JmsConsumerSettings}
...
private abstract class SourceStageLogic[T](shape: SourceShape[T],
  ...
  private[jakartajms] val handleMessage = getAsyncCallback[T] { msg =>
    if (isAvailable(out)) {
      if (queue.isEmpty) {
        pushMessage(msg)
      } else {
