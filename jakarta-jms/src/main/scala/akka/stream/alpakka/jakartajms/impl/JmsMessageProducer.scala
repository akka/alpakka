...
private class JmsMessageProducer(jmsProducer: jms.MessageProducer, jmsSession: JmsProducerSession, val epoch: Int) {
  ...
  private[jakartajms] def createMessage(element: JmsEnvelope[_]): jms.Message =
    element match {
  ...
  private[jakartajms] def populateMessageProperties(message: jakarta.jms.Message, jmsMessage: JmsEnvelope[_]): Unit =
  ...
  private def populateMapMessage(message: jakarta.jms.MapMessage, jmsMessage: JmsMapMessagePassThrough[_]): Unit =
