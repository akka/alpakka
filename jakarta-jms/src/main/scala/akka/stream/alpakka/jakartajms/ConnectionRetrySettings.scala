...
object ConnectionRetrySettings {
  val configPath = "alpakka.jakarta-jms.connection-retry"
  ...
  /**
   * Reads from the default config provided by the actor system at `alpakka.jakarta-jms.connection-retry`.
   *
  ...
  /**
   * Java API: Reads from the default config provided by the actor system at `alpakka.jakarta-jms.connection-retry`.
   *
   * @param actorSystem The actor system
   */
