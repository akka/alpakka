...
import akka.stream.alpakka.jakartajms.scaladsl.JmsConnectorState._
import akka.stream.alpakka.jakartajms.scaladsl.{JmsConnectorState, JmsConsumer, JmsProducer, JmsProducerStatus}
...
class JmsConnectionStatusSpec extends JmsSpec {
  ...
  "JmsConnector connection status" should {
    ...
    "retry connection when creating producer fails" in withMockedProducer { ctx =>
      import ctx._
      val connectAttempts = new AtomicInteger()
      when(session.createProducer(any[jakarta.jms.Destination])).thenAnswer(new Answer[MessageProducer]() {
    ...
    "retry connection when creating producer destination fails" in withMockedProducer { ctx =>
      import ctx._
      val connectAttempts = new AtomicInteger()
      when(session.createQueue(any[String])).thenAnswer(new Answer[jakarta.jms.Queue]() {
        override def answer(invocation: InvocationOnMock): jakarta.jms.Queue =
    ...
    "retry connection when creating consumer fails" in withMockedConsumer { ctx =>
      import ctx._
      val connectAttempts = new AtomicInteger()
      when(session.createConsumer(any[jakarta.jms.Destination])).thenAnswer(new Answer[MessageConsumer]() {
