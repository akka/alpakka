...
import akka.stream.alpakka.jakartajms.javadsl.JmsProducerStatus;
...
public class JmsConnectorsTest {
  ...
  public void publishAndConsumeJmsTextMessage() throws Exception {
    withServer(
        server -> {
          ...
          // #text-sink
          // #text-source
          jakarta.jms.ConnectionFactory connectionFactory = server.createConnectionFactory();
          // #text-source
          // #connection-factory
          // #text-sink
  ...
  public void publishAndConsumeJmsTextMessagesWithProperties() throws Exception {
    withServer(
        server -> {
          ...
          // #jms-source
          Source<jakarta.jms.Message, JmsConsumerControl> jmsSource =
              JmsConsumer.create(
                  JmsConsumerSettings.create(system, connectionFactory).withQueue("test"));
  ...
  // #custom-destination
  Function<jakarta.jms.Session, jakarta.jms.Destination> createQueue(String destinationName) {
    return (session) -> {
  ...
  public void browse() throws Exception {
    withServer(
        server -> {
          ...
          // #browse-source
          Source<jakarta.jms.Message, NotUsed> browseSource =
              JmsConsumer.browse(
    ...
    withServer(
        server -> {
          ...
          Destination tempQueueDest =
              akka.stream.alpakka.jakartajms.Destination.createDestination(tempQueue);
