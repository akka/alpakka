...
object SqsPublishFlow {
   ...
   */
  def apply(queueUrl: String, settings: SqsPublishSettings)(implicit
   ...
   * creates a [[akka.stream.scaladsl.Flow Flow]] to publish messages to a SQS queue using an [[software.amazon.awssdk.services.sqs.SqsAsyncClient SqsAsyncClient]]
   */
  def apply(queueUrl: String)(implicit
      sqsClient: SqsAsyncClient
  ): Flow[SendMessageRequest, SqsPublishResult, NotUsed] =
   ...
   */
  def apply(settings: SqsPublishSettings = SqsPublishSettings.Defaults)(implicit
      sqsClient: SqsAsyncClient
  ): Flow[SendMessageRequest, SqsPublishResult, NotUsed] = {
   ...
   */
  def batch(queueUrl: String, settings: SqsPublishBatchSettings = SqsPublishBatchSettings.Defaults)(implicit
      sqsClient: SqsAsyncClient
  ): Flow[Iterable[SendMessageRequest], List[SqsPublishResultEntry], NotUsed] = {
    SqsAckFlow.checkClient(sqsClient)
    Flow[Iterable[SendMessageRequest]]
      .map { requests =>
        val entries = requests.zipWithIndex.map { case (r, i) =>
      ...
      .mapAsync(settings.concurrentRequests) { case (requests, batchRequest) =>
        sqsClient
          ...
          .map {
            case response if response.failed().isEmpty =>
              ...
              requests.zipWithIndex.map { case (r, i) =>
