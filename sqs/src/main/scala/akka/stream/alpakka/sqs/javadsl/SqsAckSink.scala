...
object SqsAckSink {
  ...
  /**
   * Java API: creates a sink based on [[akka.stream.alpakka.sqs.SqsAckFlowStage]] for a SQS queue using an [[com.amazonaws.services.sqs.AmazonSQSAsync]]
   */
  ...
  /**
   * Java API: creates a sink based on [[akka.stream.alpakka.sqs.SqsAckFlowStage]] for a SQS queue using an [[com.amazonaws.services.sqs.AmazonSQSAsync]] with default settings.
   */
