...
lazy val googleCloudPubSubGrpc = alpakkaProject(
  ...
  Compile / scalacOptions += "-P:silencer:pathFilters=src_managed",
  crossScalaVersions --= Seq(Dependencies.Scala211), // 2.11 is not supported since Akka gRPC 0.6
  inTask(doc)(PB.generate / excludeFilter := new SimpleFileFilter((file: File) => {
    val excludeDirs = Set(
      "google/api",
      "google/cloud/audit",
      "google/geo",
    ...
    excludeDirs.exists(p => parentDir.contains(p))
  })) // These get generated by `googleCloudBigQueryStorage` and clash during the `doc` task
)
// #grpc-plugins
