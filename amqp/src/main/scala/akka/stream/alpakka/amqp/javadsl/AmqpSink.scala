...
object AmqpSink {
  ...
  /**
   * Java API: Creates an [[AmqpSink]] that accepts [[akka.stream.alpakka.amqp.WriteMessage WriteMessage]] elements.
   *
   ...
   */
  def create(settings: AmqpSinkSettings): akka.stream.javadsl.Sink[WriteMessage, CompletionStage[Done]] =
