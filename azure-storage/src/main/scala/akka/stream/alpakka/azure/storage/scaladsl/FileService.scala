...
object FileService {
   ...
   * @return A [[akka.stream.scaladsl.Source Source]] of [[akka.stream.alpakka.azure.storage.FileShareEntry]] elements
  ...
  def listFiles(objectPath: String, requestBuilder: ListFiles): Source[FileShareEntry, NotUsed] =
    AzureStorageStream.listFiles(objectPath, requestBuilder)
