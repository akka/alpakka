...
class SessionSpec extends FlatSpec with ScalaFutures with MockitoSugar with BeforeAndAfterAll with Matchers {
  ...
  it should "ask for new session on first request" in {
    new Fixtures {
      ...
      when(mockHttpApi.getAccessToken(TestCredentials.clientEmail, TestCredentials.privateKey,
          Instant.ofEpochSecond(0))).thenReturn(Future.successful(AccessTokenExpiry(accessToken, 3600)))
  ...
  it should "uses the cached value for the second request" in {
    new Fixtures {
      val session = new Session(TestCredentials.clientEmail, TestCredentials.privateKey) {
        override def now = Instant.ofEpochSecond(0)
        override val httpApi = mockHttpApi
  ...
  it should "requests a new session if current is about to expire" in {
    new Fixtures {
      val session = new Session(TestCredentials.clientEmail, TestCredentials.privateKey) {
