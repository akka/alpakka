...
class GooglePubSubSpec extends FlatSpec with MockitoSugar with ScalaFutures with Matchers {
  ...
  it should "auth and publish the message" in {
    new Fixtures {
  ...
  it should "subscribe and pull a message" in {
    new Fixtures {
      val message =
        ReceivedMessage(ackId = "1", message = PubSubMessage(messageId = "1", data = base64String("Hello Google!")))

      when(auth.getToken()).thenReturn(Future.successful("ok"))
      when(mockHttpApi.pull(project = TestCredentials.projectId, subscription = "sub1",
          apiKey = TestCredentials.apiKey, accessToken = "ok"))
  ...
  it should "auth and acknowledge a message" in {
    new Fixtures {
      when(auth.getToken()).thenReturn(Future.successful("ok"))
