...
object InfluxDBSink {
  ...
  def apply(settings: InfluxDBSettings)(implicit influxDB: InfluxDB):
    Sink[InfluxDBWriteMessage[Point,NotUsed],Future[Done]] =
    InfluxDBFlow.create[Point](settings).toMat(Sink.ignore)(Keep.right)

  def typed[T](
      clazz: Class[T], settings: InfluxDBSettings
