...
This connector provides two different types of factories. Factories that take an `ActorSystem` as a parameter can be
found in @scaladoc[S3](akka.stream.alpakka.s3.scaladsl.S3$). `ActorSystem` is used to read the settings from the
`application.conf` file.

It is also possible to provide a programmatically configured @scaladoc[S3Client](akka.stream.alpakka.s3.S3Client)
to factories in @scaladoc[S3External](akka.stream.alpakka.s3.scaladsl.S3External$).
