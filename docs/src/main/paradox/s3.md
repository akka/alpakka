...
final CompletionStage<MultipartUploadResult> resultCompletionStage = client.multipartCopy(bucket(), bucketKey(), 
                            targetBucket(), targetBucketKey(), S3Headers.empty(), ServerSideEncryption.AES256$.MODULE$);
```
...
```java
final CompletionStage<MultipartUploadResult> resultCompletionStage = client.multipartCopy(bucket(), bucketKey(), 
                            targetBucket(), targetBucketKey(), S3Headers.apply(CannedAcl.Private$.MODULE$), null);
```
