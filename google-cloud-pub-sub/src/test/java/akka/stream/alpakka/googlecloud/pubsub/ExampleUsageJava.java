...
public class ExampleUsageJava {
  ...
  private static void example() throws NoSuchAlgorithmException, InvalidKeySpecException {
    ...
    // #init-credentials
    String keyString =
        "MIIEvgIBADANBgkqhkiG9w0BAQEFAASCBKgwggSkAgEAAoIBAQCxwdLoCIviW0BsREeKzi"
    ...
    Flow<PublishRequest, List<String>, NotUsed> publishFlow =
        GooglePubSub.publish(
            projectId, apiKey, clientEmail, privateKey, topic, 1, system, materializer);
    ...
    Source<ReceivedMessage, NotUsed> subscriptionSource =
        GooglePubSub.subscribe(projectId, apiKey, clientEmail, privateKey, subscription, system);

    Sink<AcknowledgeRequest, CompletionStage<Done>> ackSink =
        GooglePubSub.acknowledge(
            projectId, apiKey, clientEmail, privateKey, subscription, 1, system, materializer);
