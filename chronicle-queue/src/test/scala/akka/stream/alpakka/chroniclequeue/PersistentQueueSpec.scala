...
class PersistentQueueSpec extends FlatSpec with Matchers with PrivateMethodTester {
  ...
  implicit val serializer = ChronicleQueueSerializer[ByteString]()
  ...
  it should "dequeue the same entry as it enqueues" in {
    ...
    val queue = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile))
  ...
  it should "dequeue the same entry as it enqueues for n outputPorts" in {
    val tempPath = Files.createTempDirectory("persistent_queue")
    val queue = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile, outputPorts = 3))
    val element = ByteString("Hello!")
    queue.enqueue(element)
  ...
  it should "dequeue each entry as it enqueues, for all entries" in {
    val tempPath = Files.createTempDirectory("persistent_queue")
    val queue = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile, outputPorts = 2))
    for { i <- 1 to 1000000 } {
      val element = ByteString(s"Hello $i")
      queue.enqueue(element)
    }
  ...
  it should "dequeue each entry as it enqueues, even if the stream is reopened" in {
    ...
    val queue2 = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile))
    for { i <- 1 to 700000 } {
      val element = ByteString(s"Hello $i")
    ...
    val queue3 = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile))
    for { i <- 1 to 500000 } {
      val element = ByteString(s"Hello $i")
    ...
    val queue4 = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile))
    ...
    val queue5 = new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile))
  ...
  it should "throw the appropriate exception if queue file cannot be created" in {
    ...
    a[FileNotFoundException] should be thrownBy new PersistentQueue[ByteString](ChronicleQueueConfig(badPath.toFile))
  ...
  it should "clean up buffer resources automatically" in {
    val tempPath = Files.createTempDirectory("persistent_queue")
    val queue =
      new PersistentQueue[ByteString](ChronicleQueueConfig(tempPath.toFile, rollCycle = RollCycles.TEST_SECONDLY))
