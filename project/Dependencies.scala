...
object Dependencies {
  ...
  val JakartaJms = Seq(
    libraryDependencies ++= Seq(
        ...
        ("org.apache.activemq" % "artemis-jakarta-server" % "2.36.0" % Test).exclude("org.slf4j", "slf4j-api"),
        ("org.apache.activemq" % "artemis-jakarta-client" % "2.36.0" % Test).exclude("org.slf4j", "slf4j-api")
      ) ++ Mockito
  )
